...
CREATE TABLE zoo_votes (
  ...
  CONSTRAINT unique_daily_vote UNIQUE (zoo_id, voter_id, EXTRACT(DATE FROM voted_at))
);
...
CREATE INDEX zoo_votes_voter_date_idx ON zoo_votes (voter_id, voted_at);
CREATE INDEX zoo_votes_zoo_date_idx ON zoo_votes (zoo_id, voted_at);
